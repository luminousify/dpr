...
<div class="row wrapper border-bottom white-bg page-heading">
                <div class="col-lg-10">
                    ...
                    <div class="row" style="margin-left:2px;">
                        <div class="col-sm3"> <b>Pilih Tahun</b> 
                        <select name="tahun" value="<?= $tahun; ?>" class="form-control">
                        <?php
                        $tahuns = date('Y')-3;
                        $now=date('Y');
...
<?php
    for($i = 1; $i <= 12 ; $i++)
    {
        echo 'SUM(CASE WHEN pp.bulan = '.$i.' THEN pp.qty_ok ELSE 0 END ) AS bO'.$i.',<br/>
        SUM(CASE WHEN pp.bulan = '.$i.' THEN pp.qty_ng ELSE 0 END ) AS bN'.$i.',<br/>
        SUM(CASE WHEN (bulan = '.$i.') THEN pp.`max_CTStd` ELSE NULL END) AS s'.$i.',<br/>
        SUM(CASE WHEN (bulan = '.$i.') THEN pp.`G_prod` ELSE NULL END) AS sG'.$i.',<br/>
        SUM(CASE WHEN (bulan = '.$i.') THEN pp.`N_prod` ELSE NULL END) AS sN'.$i.',<br/>';
