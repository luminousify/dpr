<title>DPR | 7 Data & 7 Table</title>
        ...
        <div class="col-lg-12">
            <div class="ibox ">
                ...
                <div class="ibox-content">
                    <?= form_open('c_report/seven_data_and_table'); ?>  
                    ...
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                        <li class="nav-item" role="presentation">
                            <a class="nav-link active" id="7data-tab" data-toggle="tab" href="#7data" role="tab" aria-controls="7data" aria-selected="true">
                                <strong>7 Data</strong>
                            </a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link" id="7table-tab" data-toggle="tab" href="#7table" role="tab" aria-controls="7table" aria-selected="false">
                                <strong>7 Table</strong>
                            </a>
                        </li>
                    </ul>
                    ...
                    <div class="tab-content pt-3" id="myTabContent">
                        <!-- 7 Data Tab -->
                        <div class="tab-pane fade show active" id="7data" role="tabpanel" aria-labelledby="7data-tab">
                            ...
                            <table class="table table-striped table-bordered table-hover dataTables-example7data" style="width:100%">
                                <thead>
                                    <tr>
                        ...
                        <!-- 7 Table Tab -->
                        <div class="tab-pane fade" id="7table" role="tabpanel" aria-labelledby="7table-tab">
                            <table class="table table-striped table-bordered table-hover dataTables-example7table" style="width:100%">
                                <thead>
                                    <tr>
                                        <th class="text-center">Mach</th>
                                        <th class="text-center">Total Of Mach Eff Hr</th>
                                ...
                                <tbody>
                                    <?php 
                                    foreach ($seventable_summary->result_array() as $row) {
                                        echo '<tr>';
                                        echo '<td class="text-center">' . $row['Mach'] . '</td>';
                                        echo '<td class="text-center">' . $row['total_mach_eff_hr'] . '</td>';
                                        echo '<td class="text-center">' . $row['01'] . '</td>';
                                        echo '<td class="text-center">' . $row['02'] . '</td>';
                                        echo '<td class="text-center">' . $row['03'] . '</td>';
                                        echo '<td class="text-center">' . $row['04'] . '</td>';
                                        echo '<td class="text-center">' . $row['05'] . '</td>';
                                        echo '<td class="text-center">' . $row['06'] . '</td>';
                                        echo '<td class="text-center">' . $row['07'] . '</td>';
                                        echo '<td class="text-center">' . $row['08'] . '</td>';
                                        echo '<td class="text-center">' . $row['09'] . '</td>';
...
<script>
    $(document).ready(function(){
        // Initialize DataTable for 7 Data tab
        $('.dataTables-example7data').DataTable({
            pageLength: 10,
            responsive: false,
            scrollX: true,
            scrollCollapse: true,
            ...
            buttons: [
                {extend: 'copy'},
                {extend: 'csv'},
                {extend: 'excel', title: 'Report7Data'},
                {extend: 'pdf', title: 'Report7Data'},
                {
                    extend: 'print',
        ...
        // Initialize DataTable for 7 Table tab
        $('.dataTables-example7table').DataTable({
            pageLength: 25,
            responsive: true,
            scrollX: true,
            scrollCollapse: true,
            ...
            buttons: [
                {extend: 'copy'},
                {extend: 'csv'},
                {extend: 'excel', title: 'Report7Table'},
                {extend: 'pdf', title: 'Report7Table'},
                {
                    extend: 'print',
                    customize: function (win){
                        $(win.document.body).addClass('white-bg');
        ...
        // Handle tab change to properly initialize DataTables in hidden tabs
        $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
            var target = $(e.target).attr("href");
            if (target === "#7table") {
                // Redraw the 7 Table DataTable when tab becomes visible
                $('.dataTables-example7table').DataTable().columns.adjust().responsive.recalc();
            } else if (target === "#7data") {
                // Redraw the 7 Data DataTable when tab becomes visible
                $('.dataTables-example7data').DataTable().columns.adjust().responsive.recalc();
            }
        });
    });
</script>
