 <title>DPR | Report Worst Defect PPM</title>
...
<div class="wrapper wrapper-content animated fadeInRight">
            <div class="row">
                ...
                <div class="ibox-content">
                    <?= form_open('c_report/worst_defect_by_ppm'); ?>  
                    ...
                    <div class="tabs-container">
                      <div class="table-responsive">
                        ...
                        <br/>
                        <?php foreach ($worst_10_ng->result_array() as $total) : ?>  
                        <?php endforeach; ?>
                        <?php foreach ($total_prod_bymonth->result_array() as $tpb) : ?>  
                        <?php endforeach; ?>
                        <?php foreach ($total_bylimit->result_array() as $tl) : ?>  
                        <?php endforeach; ?>
                        <table class="table table-bordered" id="datatable4" style="display: none;" >
                                    ...
                                    <tbody>
                                        <?php foreach($worst_10_ng->result_array() as $data_grafik)
                                            { 
                                                 echo '<tr>';
                                                 echo '<th>'.$data_grafik['nama_product'].'</th>';
                                                 echo '<td>'.round($data_grafik['qty_ng']).'</td>';
                                                echo '</tr>';
                                            }?>
                                ...
                                <table class="table table-striped table-bordered table-hover dataTables-example2" style="width: 100%">
                                    ...
                                    <tbody>
                                        <?php 
                                            $ppm = ($tpb['total_prod'] != 0) ? ($tpb['total_ng'] / $tpb['total_prod']) *1000000 : 0; 
                                            $ppm_fix = round($ppm);
                                            $no = 0 ; foreach($total_prod_bymonth->result_array() as $header)
                                            {   $no++;
                                                echo '<tr>';
                                                echo '<td>'.$no;'</td>';
                                                echo '<td>'.$header['tahun'],'</td>';
                                                echo '<td>'.$header['bulan'],'</td>';
                                                echo '<td>'.$header['total_prod'],'</td>';
                                                echo '<td>'.$header['total_ng'],'</td>';
                                                echo '<td>'.$ppm_fix;'</td>';
                                                echo '</tr>';
                                            }  ?>
                                    </tbody>   
                            ...
                            <br/>
                                <table class="table table-striped table-bordered table-hover dataTables-example" width="100%">
                                    ...
                                    <tbody>
                                        <?php 
                                            $no = 0 ; foreach($worst_10_ng->result_array() as $data)
                                            {   $no++;
                                                echo '<tr>';
                                                echo '<td>'.$no;'</td>';
                                                ...
                                                echo '<td>'.round(($data['qty_ng'] / $tpb['total_prod']) * 1000000);'</td>';
                                                echo '<td></td>';
                                                echo '<td></td>';
                                                echo '<td></td>';
                                                echo '<td></td>';
                                            ...
                                            <?php 
                                            $ppm = ($tpb['total_prod'] != 0) ? ($tl['total_ng'] / $tpb['total_prod']) *1000000 : 0;
                                            $ppm_fix = round($ppm);
                                            foreach($total_bylimit->result_array() as $total_limit)
                                            {  
                                                echo '<tr>';
                                                echo '<td style="background:#1ab394;color: white;"></td>';
                                                echo '<td style="background:#1ab394;color: white;">Total</td>';
                                                echo '<td style="background:#1ab394;color: white;"></td>';
                                                echo '<td style="background:#1ab394;color: white;"></td>';
                                                echo '<td style="background:#1ab394;color: white;">'.$total_limit['total_prod'],'</td>';
                                                echo '<td style="background:#1ab394;color: white;">'.$total_limit['total_ng'],'</td>';
                                                echo '<td style="background:#1ab394;color: white;">'.$ppm_fix;'</td>';
                                                echo '<td style="background:#1ab394;color: white;"></td>';
                                                echo '<td style="background:#1ab394;color: white;"></td>';
                                                echo '<td style="background:#1ab394;color: white;"></td>';
                                                echo '<td style="background:#1ab394;color: white;"></td>';
...
$(document).ready(function() {         
   ...
   options = {
         ...
         title: {
            text: 'Production Quantity Data Year <?= $tahun; ?>'
         },
         xAxis: {
                    //gridLineWidth: 1,
                    labels: {
         ...
         colors: [ '#4ec4ce', '#f7941d', '#cc113c',  '#00ff00', '#800080'],
         plotOptions: {
        column: {
            dataLabels: {
                enabled: true
...
$(document).ready(function() {         
   ...
   options = {
         ...
         title: {
            text: '10 WORST DEFECT BY PPM <?= $bulan; ?> - <?= $tahun; ?>'
         },
         xAxis: {
                    //gridLineWidth: 1,
                    labels: {
         ...
         colors: [ '#4ec4ce', '#cc113c' , '#f7941d' ,  'black'],
         plotOptions: {
        column: {
            dataLabels: {
                enabled: true
